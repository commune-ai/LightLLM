...
from engine.fabric.fabric import Fabric
from engine.fabric.strategies import FSDPStrategy
...
from llama_model.model import Block, LLaMA, LLaMAConfig
from llama_model.utils import save_model_checkpoint
...
def main() -> None:
    ...
    fabric = Fabric(accelerator="cuda", devices=4, precision="bf16-mixed", strategy=strategy)
    fabric.launch()
...
@torch.no_grad()
def validate(fabric: Fabric, model: torch.nn.Module, val_data: np.ndarray) -> torch.Tensor:
...
def get_batch(fabric: Fabric, data: np.ndarray, block_size: int) -> Tuple[torch.Tensor, torch.Tensor]:
