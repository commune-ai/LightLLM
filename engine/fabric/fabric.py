...
import engine.fabric
from engine.fabric.accelerators.accelerator import Accelerator
from engine.fabric.connector import _PLUGIN_INPUT, _PRECISION_INPUT, _Connector, _is_using_cli
from engine.fabric.loggers import Logger
from engine.fabric.plugins import Precision  # avoid circular imports: # isort: split
from engine.fabric.strategies import (
...
from engine.fabric.strategies.fsdp import _has_meta_device_parameters
from engine.fabric.strategies.launchers import _MultiProcessingLauncher, _XLALauncher
from engine.fabric.strategies.strategy import TBroadcast, _Sharded
from engine.fabric.utilities import move_data_to_device
from engine.fabric.utilities.apply_func import convert_tensors_to_scalars, convert_to_tensors
from engine.fabric.utilities.data import (
    _auto_add_worker_init_fn,
    _replace_dunder_methods,
    _update_dataloader,
    has_iterable_dataset,
)
from engine.fabric.utilities.device_dtype_mixin import _update_properties
from engine.fabric.utilities.distributed import DistributedSamplerWrapper, _InfiniteBarrier
from engine.fabric.utilities.imports import _TORCH_GREATER_EQUAL_2_0
from engine.fabric.utilities.rank_zero import rank_zero_deprecation, rank_zero_warn
from engine.fabric.utilities.registry import _load_external_callbacks
from engine.fabric.utilities.seed import seed_everything
from engine.fabric.utilities.types import ReduceOp
from engine.fabric.utilities.warnings import PossibleUserWarning
from engine.fabric.wrappers import (
    _FabricDataLoader,
    _FabricModule,
    _FabricOptimizer,
    _to_compiled,
...
class Fabric:
    ...
    Args:
        ...
        callbacks: A single callback or a list of callbacks. A callback can contain any arbitrary methods that
            can be invoked through :meth:`~fabric.fabric.Fabric.call` by the user.
        loggers: A single logger or a list of loggers. See :meth:`~fabric.fabric.Fabric.log` for more
            information.
    ...
    def backward(self, tensor: Tensor, *args: Any, model: Optional[_FabricModule] = None, **kwargs: Any) -> None:
        ...
        engine.fabric.wrappers._in_fabric_backward = True
        try:
            self._strategy.backward(tensor, module, *args, **kwargs)
        finally:
            engine.fabric.wrappers._in_fabric_backward = False
    ...
    def seed_everything(seed: Optional[int] = None, workers: Optional[bool] = None) -> int:
        r"""Helper function to seed everything without explicitly importing LightLLM.

        See :func:`~fabric.utilities.seed.seed_everything` for more details.
    ...
    def _configure_callbacks(callbacks: Optional[Union[List[Any], Any]]) -> List[Any]:
        ...
        callbacks.extend(_load_external_callbacks("fabric.callbacks_factory"))
        return callbacks
