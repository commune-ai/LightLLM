...
import engine.fabric as fabric
from engine.fabric.accelerators import XLAAccelerator
from engine.fabric.plugins.precision import XLAPrecision
from engine.fabric.strategies import _StrategyRegistry
from engine.fabric.strategies.single_xla import SingleDeviceXLAStrategy
from engine.fabric.utilities.rank_zero import rank_zero_deprecation


def _patch_sys_modules() -> None:
    self = sys.modules[__name__]
    sys.modules["engine.fabric.strategies.single_tpu"] = self
    sys.modules["engine.fabric.accelerators.tpu"] = self
    sys.modules["engine.fabric.plugins.precision.tpu"] = self
    sys.modules["engine.fabric.plugins.precision.tpu_bf16"] = self
    sys.modules["engine.fabric.plugins.precision.xlabf16"] = self


class SingleTPUStrategy(SingleDeviceXLAStrategy):
    """Legacy class.

    Use :class:`~engine.fabric.strategies.single_xla.SingleDeviceXLAStrategy` instead.
...
class TPUAccelerator(XLAAccelerator):
    ...
    Use :class:`~engine.fabric.accelerators.xla.XLAAccelerator` instead.

    """

    def __init__(self, *args: Any, **kwargs: Any) -> None:
        rank_zero_deprecation(
            "The `TPUAccelerator` class is deprecated. Use `engine.fabric.accelerators.XLAAccelerator` instead."
        )
        super().__init__(*args, **kwargs)
...
class TPUPrecision(XLAPrecision):
    ...
    Use :class:`~engine.fabric.plugins.precision.xla.XLAPrecision` instead.

    """

    def __init__(self, *args: Any, **kwargs: Any) -> None:
        rank_zero_deprecation(
            "The `TPUPrecision` class is deprecated. Use `engine.fabric.plugins.precision.XLAPrecision`" " instead."
        )
...
class XLABf16Precision(XLAPrecision):
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        rank_zero_deprecation(
            ...
            " `engine.fabric.plugins.precision.XLAPrecision` instead."
        )
