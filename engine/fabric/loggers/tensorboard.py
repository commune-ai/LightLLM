...
from engine.fabric.loggers.logger import Logger, rank_zero_experiment
from engine.fabric.utilities.cloud_io import _is_dir, get_filesystem
from engine.fabric.utilities.logger import _add_prefix, _convert_params, _flatten_dict
from engine.fabric.utilities.logger import _sanitize_params as _utils_sanitize_params
...
from engine.fabric.wrappers import _unwrap_objects
...
class TensorBoardLogger(Logger):
    ...
    ``os.path.join(root_dir, name, version)``. This is the recommended logger in engine Fabric.

    Args:
        root_dir: The root directory in which all your experiments with different names and versions will be stored.
        name: Experiment name. Defaults to ``'engine_logs'``. If it is the empty string then no per-experiment
    ...
    Example::
        ...
        from engine.fabric.loggers import TensorBoardLogger
