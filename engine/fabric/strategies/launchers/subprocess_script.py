...
from engine.fabric.utilities.rank_zero import rank_prefixed_message
...
class _SubprocessScriptLauncher(_Launcher):
    ...
    def _check_can_spawn_children(self) -> None:
        ...
        if self.cluster_environment.local_rank() != 0:
            raise RuntimeError(
                "engine attempted to launch new distributed processes with `local_rank > 0`. This should not happen."
...
def _hydra_subprocess_cmd(local_rank: int) -> Tuple[Sequence[str], str]:
    ...
    import __main__  # local import to avoid https://github.com/Big-Scale/LightLLM/engine/
